...
def price_to_float(price_str):
    if isinstance(price_str, str):
        ...
        # Brazilian format: 1.700,05 -> 1700.05
        # If there is no comma, assume integer
        if ',' in cleaned:
            # Remove dots (thousands) and replace comma with dot
            cleaned = cleaned.replace('.', '').replace(',', '.')
        else:
            # No comma, maybe just remove dots if they are thousands
...
output_dir = 'forecast_output_final'
